...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/sap_likey/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/sap_likey/*")
            ...
            #    "--include=src/saltext/sap_likey/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/sap_likey/config/schemas",
    )
